...
\begin{align}
\hat{\mathbf{\beta}}&=(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}\mathbf{y}\\
&=(\mathbf{X}^T\mathbf{X})^{-1}\Lambda\hat{b}
\end{align}
...
And similarly, $\hat{\sigma}^2_{\beta}=(\frac{y^Ty-\hat{\mathbf{\beta}}^T\mathbf{\Lambda}\hat{b}}{n-m})(\mathbf{X}^T\mathbf{X})^{-1}$
