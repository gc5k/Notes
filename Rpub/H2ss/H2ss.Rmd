...
$$
\begin{align}
L_B&=&tr(KK^T)=\frac{1}{B}\sum_{b=1}^Bz_b^T\mathbf{KK}^Tz_b\\
...
&=\frac{1}{B}\sum_{l_1}\sum_{l_2}\mathcal{K}^2_{l_1,l_2}
\end{aligned}$$

in which $\mathcal{K}^2=\mathbf{KK}^T$.
...
k4=k2%*%t(k2)
k4Alt=k2^2
print(paste("sum(diag(K4))=", sum(diag(k4)), "sum[(kkT)^2]=k4Alt"))
#evaluate LB
...
$Me_w$ can be defined accordingly.
```{r OmegaW}
n=500 #sample size
m=1000 #marker
h2=0.3 #heritability
