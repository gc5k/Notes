...
impF=paste0(out, ".imp")
impute=paste(gear, "--bfile", exF, "--naive-imputation --make-bed --out", impF)
system(impute)
...
#  bimB.ex=read.table(paste0(exF, ".bim"), as.is=T)
  bimB.ex=read.table(paste0(impF, ".bim"), as.is=T)
...
#  bimQ.ex=read.table(paste0(exF, ".bim"), as.is=T)
  bimQ.ex=read.table(paste0(impF, ".bim"), as.is=T)
